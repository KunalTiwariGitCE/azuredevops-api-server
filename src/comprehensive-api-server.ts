...
async function main() {
  ...
  app.get('/api/projects/:project/workitems', async (req, res) => {
    try {
      ...
      if (assignedTo) whereClause += ` AND [System.AssignedTo] = '${assignedTo}'`;
      if (state) whereClause += ` AND [System.State] = '${state}'`;
      if (type) whereClause += ` AND [System.WorkItemType] = '${type}'`;
      ...
      if (queryResult.workItems && queryResult.workItems.length > 0) {
        const ids = queryResult.workItems.map(wi => wi.id!).slice(0, parseInt(limit as string));
  ...
  app.post('/api/workitems', async (req, res) => {
    try {
      ...
      if (description) document.push({ op: "add", path: "/fields/System.Description", value: description });
      if (assignedTo) document.push({ op: "add", path: "/fields/System.AssignedTo", value: assignedTo });
      if (tags) document.push({ op: "add", path: "/fields/System.Tags", value: tags });
  ...
  app.put('/api/workitems/:id', async (req, res) => {
    try {
      ...
      const document = [];
      if (title) document.push({ op: "replace", path: "/fields/System.Title", value: title });
      if (description) document.push({ op: "replace", path: "/fields/System.Description", value: description });
      if (state) document.push({ op: "replace", path: "/fields/System.State", value: state });
      if (assignedTo) document.push({ op: "replace", path: "/fields/System.AssignedTo", value: assignedTo });
      if (tags) document.push({ op: "replace", path: "/fields/System.Tags", value: tags });
  ...
  app.get('/api/repositories/:repo/branches', async (req, res) => {
    try {
      ...
      const connection = await getAzureDevOpsClient();
      const gitApi = await connection.getGitApi();
      const refs = await gitApi.getRefs(repoId, project as string, "heads/");
  ...
  app.get('/api/repositories/:repo/pullrequests', async (req, res) => {
    try {
      ...
      const connection = await getAzureDevOpsClient();
      const gitApi = await connection.getGitApi();
      const prs = await gitApi.getPullRequests(repoId, { status: status as any }, project as string);
  ...
  app.get('/api/projects/:project/builds', async (req, res) => {
    try {
      ...
      const builds = await buildApi.getBuilds(
        projectName, 
        definition ? [parseInt(definition as string)] : undefined,
        undefined, undefined, undefined, undefined, undefined, undefined,
        status as any, undefined, undefined, undefined, parseInt(top as string)
      );
    ...
    try {
      ...
      res.json({
        ...
        data: definitions.map(d => ({
          ...
          type: d.type,
          path: d.path,
          repository: (d as any).repository?.name,
